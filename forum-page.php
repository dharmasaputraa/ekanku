...
$ambilNamaUser = mysqli_fetch_array($cekUser);
$namaUser = $ambilNamaUser['nickname'];

if (isset($_GET['forum'])) {
    $idPertanyaan = $_GET['forum'];

    $query = "SELECT * FROM forum LEFT JOIN user ON user.id = forum.user WHERE id_forum = '$idPertanyaan';";
    $sql = mysqli_query($conn, $query);

    $result = mysqli_fetch_assoc($sql);

    $namaUser = $result['nickname'];
    $title = $result['title'];
    $date = $result['date'];
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;900&family=Open+Sans:wght@300;400&display=swap" rel="stylesheet" />
...
<body class="bg-light">
    <!-- Navbar -->
    <nav class="navbar navbar-expand-lg navbar-light py-0 bg-white position-fixed w-100 shadow-sm rounded mx-auto">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item px-1">
                        <a class="nav-link" href="index.php">Pelatihan</a>
                    </li>
                    <li class="nav-item x-1">
                ...
                <form class="d-flex w-50 ms-lg-auto py-2 py-lg-0">
                    ...
                    <button class="btn-search border border-lights" type="submit">
                ...
                <div class="ms-lg-3 py-2 py-lg-0">
                    ...
                    ?>
                        <form action="#" method="POST">
                            <button class="btn-login" id="btnLogin" name="logout" type="submit">
                                Logout
                            </button>
                        </form>
                    <?php
    ...
    <div class="pt-5 container min-vh-100">
        <div class="d-flex py-0">
            <div class="col-2">
                <div class="pt-5 mx-2">
                    <div class="card d-flex align-items-center justify-content-start py-3 px-3">
                        <img src="assets/image/user.svg" alt="" height="40" class="m-2" />
                        <h1 class="fs-6 fw-bold mt-1"><?= $_SESSION['nama']; ?></h1>
                    </div>
                </div>
                <div class="pt-2 mx-2">
                    <div class="card d-flex justify-content-start py-3 px-4">
                        <div class="">
                            <p class="d-flex justify-content-start mb-1">
                                <span class="subheading fw-bold text-dark">PERTANYAAN</span>
                            </p>
                            <div class="my-2">
                                <a href="#" class="subheading text-dark">
                                    <i class="uil-comment-alt-question fs-6 me-1"></i>Semua Pertanyaan
                                </a>
                            </div>
                            <div class="my-2">
                                <a href="#" class="subheading text-dark">
                                    <i class="uil uil-filter fs-6 me-1"></i>
                                    Filter
                                </a>
                            </div>
                        </div>
                ...
                <div class="pt-5 mx-2">
                    <div class="card p-0">
                        <div class="d-flex align-items-center justify-content-between py-3 px-4">
                            <div>
                                <span class="text-dark fs-5 fw-bold"><?= $title; ?></span>
                            </div>
                            <?php if ($namaUser == $_SESSION['nama']) { ?>
                                <div>
                                    <a href="forum/edit.php?ubah=<?= $result['id_forum']; ?>" class="subheading fs-6"><span class="text-dark"><i class="uil uil-pen me-1"></i>Edit</span></a>
                                    <a href="forum/forum-proses.php?hapus=<?= $result['id_forum']; ?>" type="button" name="hapus" class="fs-5 text-danger" onClick="return confirm('Apakah anda ingin menghapus data ini?')"><i class='uil-trash ms-2'></i></a>
                                </div>
                            <?php } ?>
                        </div>
                        <div class="d-flex align-items-center justify-content-between px-4">
                            <div class="border-bottom vw-100">
                                <div class="d-flex align-items-center justify-content-end"></div>
                                <div>
                                    <p class="subheading text-dark mt-2">
                                        <?= $detail; ?>
                                    </p>
                                </div>
                                <div class="border-bottom pb-3">
                                    <div class="d-flex align-items-center justify-content-between mt-4">
                                        <p class="mb-0 subheading">
                                            <i class="uil uil-user me-1"></i><?= $namaUser; ?>,
                                            <?= $date; ?>
                                        </p>
                                        <div class="d-flex justify-content-end subheading">
                                            <p class="mb-0 me-3">
                                                <i class="uil uil-eye me-1 fs-6"></i>0 View
                                            </p>
                                            <p class="mb-0">
                                                <i class="uil uil-comment me-1 fs-6"></i>0 Answer
                                            </p>
                                        </div>
                                    </div>
                                    <a href="#" class="subheading pb-1 text-dark mb-3"><i class="uil uil-comment me-1"></i>Balas</a>
                                </div>
                                <div class="py-3 border-bottom">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <p class="text-dark subheading fw-bold mb-0">
                                            Dharma Saputra
                                        </p>
                                        <a href="pembayaran/index.html" class="subheading"><i class="uil-gift me-1"></i>Beri imbalan</a>
                                    </div>

                                    <p class="subheading my-1">
                                        Kurang tau :)
                                    </p>
                                    <a href="#" class="subheading my-3 text-dark"><i class="uil uil-comment me-1"></i>Balas</a>
                                </div>
                            </div>
                        </div>
                    </div>
